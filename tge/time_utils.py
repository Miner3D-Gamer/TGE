...
class Timer:
    ...
    def __init__(
        self, start_time: Union[int, float], offset: Union[int, float] = 0
    ) -> None:
        ...
        """
        self.start_time: Union[int, float] = start_time
        self.offset: Union[int, float] = offset
...
class TimerManager:
    ...
    def remove_time_from_timer(
        self, timer_name: str, amount: Union[int, float]
    ) -> float:
...
def get_date() -> Tuple[int, int, int, int, int, int, int]:
    ...
    new_date = (
        str(date).replace("-", " ").replace(":", " ").replace(".", " ").split(" ")
    )
    return (
        int(new_date[0]),
        int(new_date[1]),
